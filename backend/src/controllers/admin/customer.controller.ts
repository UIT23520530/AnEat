...
/**
 * Get all customers (Admin view - all branches)
...
export const getAllCustomers = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const branchId = req.query.branchId as string; // Admin can filter by branch
    ...
    const { customers, total } = await CustomerService.findAll({
      ...
      search,
      tier,
      branchId, // Pass branchId for filtering
    });
...
/**
 * Get customer statistics (Admin view - system-wide)
...
export const getCustomerStatistics = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const statistics = await CustomerService.getStatistics(branchId);
 ...
 * POST /api/v1/admin/customers/:id/adjust-points
 */
...
/**
 * Create new customer (manual registration by admin)
...
/**
 * Get customer stats (Admin-specific - more detailed)
